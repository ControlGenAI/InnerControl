...
def main(args):
    ...
    ######################################
    readout_control = get_reward_model('hed', 'https://huggingface.co/lllyasviel/Annotators/resolve/main/ControlNetHED.pth')
    ...
    readout_control.requires_grad_(False)
    controlnet.train()
    ...
    config = {'rg_kwargs': [
                            ...
                            #  {'head_type': 'spatial', 
                            #  'loss_rescale': 0.5, 
                            #  'aggregation_kwargs': {'aggregation_ckpt': args.readout_path_1,
                            #  }}
                             ]
                             }
    ...
    readout_control.to(accelerator.device, dtype=weight_dtype)
    #loss_fn_vgg.to(accelerator.device, dtype=weight_dtype)
    reward_model.eval()
    readout_control.eval()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        reward_step_lpips_loss_per_epoch = 0.
        reward_step_cosine_loss_per_epoch = 0.
        reward_step_edges_loss_per_epoch = 0.
        ...
        train_reward_step_lpips_loss = 0.
        train_reward_step_cosine_loss = 0.
        train_reward_edges_step_loss = 0.
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                #print(controlnet_image.min(), controlnet_image.max())
                # if random.random() < 0.:
                #     #noise = torch.randn_like(controlnet_image) * args.noise_std
                #     #controlnet_image += noise
                ...
                Training ControlNet
                """
                ######### prepare readout control #################
                input_image = batch["pixel_values"].to(dtype=weight_dtype)
                edges_hed = readout_control(((input_image + 1) / 2).to(accelerator.device))
                edges_hed = label_transform(edges_hed, 'hed', 'limingcv/MultiGen-20M_train')
                ...
                # p=0.
                # if a < p:
                #     controlnet_image = torch.cat([controlnet_image, torch.zeros_like(controlnet_image)])
                ...
                for i in range(len(edits)):
                    ...
                    aggregation_network = edits[i]["aggregation_network"]
                    ...
                    if types[i] == 'depth':
                        ...
                        # assert obs_feat.min() >= 0 and obs_feat.max() <= 1(torchvision.transforms.functional.resize(obs_feat, (512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(labels.unsqueeze(1).repe
                        ...
                        #reward_step_lpips_loss = loss_fn_vgg(torchvision.transforms.functional.resize(obs_feat, (512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(labels.to(obs_feat.dtype).unsqueeze(1).repeat(1,3,1,1), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR))
                        reward_step_cosine_loss =  1 - F.cosine_similarity(torchvision.transforms.functional.resize(obs_feat.float(), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).view(obs_feat.size(0), -1), 
                                                         torchvision.transforms.functional.resize(labels.unsqueeze(1).repeat(1,3,1,1), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).float().view(obs_feat.size(0), -1), 
                                                        dim=1) 
                        reward_step_loss = F.mse_loss(torchvision.transforms.functional.resize(obs_feat.float(), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(labels.unsqueeze(1).repeat(1,3,1,1), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).float(), reduction="none").mean(dim=(-1,-2, -3))
                        reward_output_loss = F.mse_loss(torchvision.transforms.functional.resize(obs_feat.float(), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(outputs.detach().unsqueeze(1).repeat(1,3,1,1), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).float(), reduction="none").mean(dim=(-1,-2, -3))
                        reward_output_loss_1 = F.mse_loss(torchvision.transforms.functional.resize(obs_feat.detach().float(), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(outputs.unsqueeze(1).repeat(1,3,1,1), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).float(), reduction="none").mean(dim=(-1,-2, -3))
                        ...
                        obs_feat =  (obs_feat + 0.5).clip(0, 1)
                        assert obs_feat.min() >= 0 and obs_feat.max() <= 1
                        #print(obs_feat.shape, edges_hed.shape)
                        reward_step_cosine_loss = 1 - F.cosine_similarity(torchvision.transforms.functional.resize(obs_feat.float(), (64, 64), interpolation=transforms.InterpolationMode.BILINEAR).view(obs_feat.size(0), -1), 
                                                         torchvision.transforms.functional.resize(edges_hed.repeat(1,3,1,1), (64, 64), interpolation=transforms.InterpolationMode.BILINEAR).float().view(obs_feat.size(0), -1), 
                                                        dim=1) 
                        #reward_step_loss = F.mse_loss(torchvision.transforms.functional.resize(obs_feat.float(), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(edges_hed, (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).float(), reduction="none")
                        # min_value = 0
                        # is_zero = edges_hed == min_value
                        # if is_zero.sum() > 0:
                        #     # edges_hed_repeated = edges_hed.repeat(1, 3, 1, 1)  
                        #     # mask = (edges_hed_repeated == min_value)
                        #     reward_step_loss_final = torch.zeros(edges_hed.shape[0]).to(accelerator.device)
                        #     for i in range(len(reward_step_loss)):
                        #         reward_step_loss_final[i] = (reward_step_loss[i][edges_hed[i].repeat(3,1,1) == min_value].mean() + reward_step_loss[i][edges_hed[i].repeat(3,1,1) != min_value].mean()) / 2
                        # else:
                        #     reward_step_loss = reward_step_loss.mean(dim=(-1,-2, -3))
                        # reward_step_edges_loss = reward_step_loss_final
                ...
                # reward_step_lpips_loss = reward_step_lpips_loss.reshape_as(timestep_mask)
                # reward_step_lpips_loss = (timestep_mask * reward_step_lpips_loss).sum() / (timestep_mask.sum() + 1e-10)

                reward_step_cosine_loss = reward_step_cosine_loss.reshape_as(timestep_mask)
                reward_step_cosine_loss = (timestep_mask * reward_step_cosine_loss).sum() / (timestep_mask.sum() + 1e-10)


                # reward_step_edges_loss = reward_step_edges_loss.reshape_as(timestep_mask)
                # reward_step_edges_loss = (timestep_mask * reward_step_edges_loss).sum() / (timestep_mask.sum() + 1e-10)
                ...
                loss += (reward_step_cosine_loss) * args.readout_alpha #+ reward_step_loss_consistency * 10.
                ...
                #avg_reward_step_lpips_loss = accelerator.gather(reward_step_lpips_loss.repeat(args.train_batch_size)).mean()
                avg_reward_step_cosine_loss = accelerator.gather(reward_step_cosine_loss.repeat(args.train_batch_size)).mean()
                #avg_reward_edges_step_loss = accelerator.gather(reward_step_edges_loss.repeat(args.train_batch_size)).mean()
                ...
                #train_reward_step_lpips_loss += avg_reward_step_lpips_loss.item() / args.gradient_accumulation_steps
                train_reward_step_cosine_loss += avg_reward_step_cosine_loss.item() / args.gradient_accumulation_steps
                # train_reward_edges_step_loss += avg_reward_edges_step_loss.item() / args.gradient_accumulation_steps
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log({
                        ...
                        #"train_readout_lpips_step_loss": train_reward_step_lpips_loss,
                        "train_readout_cosine_step_loss": train_reward_step_cosine_loss,
                        # "train_reward_edges_step_loss": train_reward_edges_step_loss,
                ...
                #reward_step_lpips_loss_per_epoch += train_reward_step_lpips_loss
                reward_step_cosine_loss_per_epoch += train_reward_step_cosine_loss
                # reward_step_edges_loss_per_epoch += train_reward_edges_step_loss
            ...
            logs = {
                ...
                #"readout_lpips_step_loss": reward_step_lpips_loss.detach().item(),
                "readout_cosine_step_loss": reward_step_cosine_loss.detach().item(),
                # "train_reward_edges_step_loss": reward_step_edges_loss.detach().item(),
        ...
        logs = {
            ...
            #"reward_step_lpips_loss_per_epoch": reward_step_lpips_loss_per_epoch / len(train_dataloader),
            "reward_step_cosine_loss_per_epoch": reward_step_cosine_loss_per_epoch / len(train_dataloader),
