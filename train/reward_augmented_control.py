    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                ################################# add augmentations
                controlnet_image_readout = controlnet_image.clone() * 2 - 1
                a = random.random()
                p = 0.5
                gaussian_blur = torchvision.transforms.GaussianBlur(kernel_size=(5, 5), sigma=(1., 2.0))
                # if  a < p:  # 50% chance to apply gaussian blur
                #     controlnet_image_readout = gaussian_blur(controlnet_image_readout)
                # elif a >= p and a < p*2:  # 50% chance to add gaussian noise
                if  a < p: 
                    noise = torch.randn_like(controlnet_image_readout) * args.noise_std
                    controlnet_image_readout += noise
                    
                else:
                    pass
                
                controlnet_image_readout = controlnet_image_readout * 0.5 + 0.5
                ...
                if a < p:
                    controlnet_image = torch.cat([controlnet_image, controlnet_image_readout])
                ...
                # if a < 0:
                #     reward_step_loss_consistency = reward_step_loss_consistency.mean(dim=(-1,-2,-3))
                #     reward_step_loss_consistency = reward_step_loss_consistency.reshape_as(timestep_mask)
                #     reward_step_loss_consistency = (timestep_mask * reward_step_loss_consistency).sum() / (timestep_mask.sum() + 1e-10)
