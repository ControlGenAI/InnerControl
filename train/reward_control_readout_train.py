...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--optimize_readout",
        action="store_true",
        help="If set, optimize readout models",
    )
    parser.add_argument(
        "--readout_path",
        type=str,
...
def main(args):
    ...
    edits = get_edits(config, accelerator.device, torch.float32)
    ...
    if not args.optimize_readout:
        ...
        optimizer_readout = optimizer_class(
            edits[0]['aggregation_network'].parameters(),
            lr=0.0005,
            betas=(args.adam_beta1, args.adam_beta2),
            weight_decay=args.adam_weight_decay,
            eps=args.adam_epsilon,
    ...
    if not args.optimize_readout:
        ...
        # Create a second learning rate scheduler for the aggregation network optimizer
        lr_scheduler_readout = get_scheduler(
            args.lr_scheduler,
            optimizer=optimizer_readout,  # Assuming optimizer_edits is defined for the aggregation network
            num_warmup_steps=args.lr_warmup_steps * args.gradient_accumulation_steps,
            num_training_steps=args.max_train_steps * args.gradient_accumulation_steps,
            num_cycles=args.lr_num_cycles,
            power=args.lr_power,
        ...
        controlnet, edits[0]['aggregation_network'], optimizer, optimizer_readout, train_dataloader, lr_scheduler, lr_scheduler_readout = accelerator.prepare(
            controlnet, edits[0]['aggregation_network'], optimizer, optimizer_readout, train_dataloader, lr_scheduler, lr_scheduler_readout
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            noisy_latents_readout = noisy_latents.clone()

            if global_step % args.train_readout_steps != 0:
                with accelerator.accumulate(controlnet):
                    ...
                    # print(args.max_timestep_readout)
                    # print(timesteps, timestep_mask, timestep_mask_consistency)
                    ...
                    if args.normalize:
                        ...
                        # if a < 0:
                        #     reward_step_loss_consistency = F.mse_loss(torchvision.transforms.functional.resize(obs_feat.unsqueeze(1).float(), (64, 64), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(obs_feat_augmented.unsqueeze(1), (64, 64), interpolation=transforms.InterpolationMode.BILINEAR).float(), reduction="none")
                        # else:
                        #     reward_step_loss_consistency = 0.
                    else:
                        assert False
                ...
                #==========================
            elif global_step % args.train_readout_steps == 0:
                with accelerator.accumulate(edits[0]['aggregation_network']):
                    # normalize the predicted depth to (0, 1]
                    # map label into [0, 1]
                    labels = batch["labels"].mean(dim=1)  # (N, 3, H, W) -> (N, H, W)
                    labels_readout =  batch["labels"]
                    ...
                    #     controlnet_image_permuted = controlnet_image.clone().contiguous()[torch.randperm(controlnet_image.size(0))]
                    # else:
                    #     controlnet_image_permuted = controlnet_image.clone().contiguous()

                    controlnet_image_permuted = controlnet_image.clone().contiguous()[torch.randperm(controlnet_image.size(0))]
                    controlnet_image_permuted = torch.cat([controlnet_image_permuted, controlnet_image])
                    #controlnet_image_permuted = controlnet_image
                    ...
                    with torch.no_grad():
                        down_block_res_samples, mid_block_res_sample = controlnet(
                            torch.cat([noisy_latents_readout, noisy_latents_readout]),
                            torch.cat([timesteps, timesteps]),
                            encoder_hidden_states=torch.cat([encoder_hidden_states, encoder_hidden_states]) ,
                            controlnet_cond=controlnet_image_permuted,
                            return_dict=False,
                        )
                        # Predict the noise residual
                        model_pred = unet(
                        ...
                        obs_feat_permuted = collect_and_resize_feats(unet, None, collection_type=args.readout_type)

                    
                        model_pred = unet(
                            noisy_latents_readout,
                            timesteps.clone(),
                    ...
                    obs_feat_permuted = torch.cat([obs_feat_permuted, obs_feat])
                    emb = torch.cat([emb, emb, emb])

                    obs_feat_permuted = run_aggregation(obs_feat_permuted.float().contiguous(), edits[0]["aggregation_network"], emb).contiguous()
                    
                    timestep_mask_consistency = (args.min_timestep_readout <= timesteps.reshape(-1, 1)) & (timesteps.reshape(-1, 1) <= 400)
                    timestep_mask_consistency_unet_control = (args.min_timestep_readout <= timesteps.reshape(-1, 1)) & (timesteps.reshape(-1, 1) <= 1000)
                    timestep_mask_consistency_unet = (args.min_timestep_readout <= timesteps.reshape(-1, 1)) & (timesteps.reshape(-1, 1) <= 1000)
                    timestep_mask_consistency = torch.cat([timestep_mask_consistency, timestep_mask_consistency_unet_control, timestep_mask_consistency_unet]).contiguous()

                    obs_feat_permuted = (obs_feat_permuted + 1) / 2
                    assert labels.min() >= 0 and labels.max() <= 1
                    assert obs_feat_permuted.min() >= 0 and obs_feat_permuted.max() <= 1

                    obs_feat_permuted = obs_feat_permuted.contiguous().mean(1).contiguous()
                    max_values = obs_feat_permuted.view(obs_feat_permuted.size(0), -1).max(dim=1)[0]
                    obs_feat_permuted = obs_feat_permuted.contiguous() / max_values.view(-1, 1, 1)

                    # obs_feat_unet = obs_feat_permuted[labels.shape[0]:]
                    # obs_feat_permuted = obs_feat_permuted[:labels.shape[0]]
                    #reward_output_loss = F.mse_loss(torchvision.transforms.functional.resize(obs_feat_permuted.unsqueeze(1).float(), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(labels.detach().contiguous().unsqueeze(1), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).float(), reduction="none")
                    reward_output_loss = F.mse_loss(torchvision.transforms.functional.resize(obs_feat_permuted.unsqueeze(1).float(), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).contiguous(),  torchvision.transforms.functional.resize(torch.cat([labels, labels, labels]).contiguous().unsqueeze(1), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).float(), reduction="none")
                    reward_output_loss = reward_output_loss.mean(dim=(-1,-2,-3))
                    #reward_output_loss_unet = F.mse_loss(torchvision.transforms.functional.resize(obs_feat_unet.unsqueeze(1).float(), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(labels.detach().unsqueeze(1), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).float(), reduction="none").mean()
                    
                    reward_output_loss = reward_output_loss.reshape_as(timestep_mask_consistency).contiguous()

                    batch_size = emb.shape[0]
                    first_third = batch_size // 3
                    reward_output_loss[:first_third] *= 3
                    reward_output_loss = (timestep_mask_consistency * reward_output_loss).sum() / (timestep_mask_consistency.sum() + 1e-10)

                    #reward_output_loss += reward_output_loss_unet
                    ...
                    accelerator.backward(reward_output_loss)
                    if accelerator.sync_gradients:
                        accelerator.clip_grad_norm_(edits[0]['aggregation_network'].parameters(), args.max_grad_norm)
                    optimizer_readout.step()
                    lr_scheduler_readout.step()
            ...
            if accelerator.sync_gradients:
                ...
                if train_loss != 0:
                    ...
                    accelerator.log({
                            ...
                            "train_readout_output_loss": train_reward_output_step_loss,
                            #"train_reward_output_step_loss_1": train_reward_output_step_loss_1,
                            "lr": lr_scheduler_readout.get_last_lr()[0]
                        },
                        step=global_step
                    )
            ...
            # only show in the progress bar
                logs = {
                    ...
                    #"train_readout_output_loss": train_readout_output_loss.detach().item(),
                    "lr": lr_scheduler.get_last_lr()[0],
                    "lr readout ": lr_scheduler_readout.get_last_lr()[0]
                }
                progress_bar.set_postfix(**logs)
            except:
                logs = {
                    "train_readout_step_loss": reward_output_loss.detach().item(),
                    "lr": lr_scheduler_readout.get_last_lr()[0]
                }
                progress_bar.set_postfix(**logs)
