    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                timesteps_future = timesteps.clone()
                ...
                timestep_mask_blured = (200 <= timesteps.reshape(-1, 1)) & (timesteps.reshape(-1, 1) <= 920)
                ...
                timestep_mask_readout = (args.min_timestep_readout <= timesteps.reshape(-1, 1)) & (timesteps.reshape(-1, 1) <= args.max_timestep_readout)
                ...
                if args.normalize:
                    ...
                    reward_output_loss = F.mse_loss(torchvision.transforms.functional.resize(obs_feat.unsqueeze(1).float(), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  torchvision.transforms.functional.resize(labels.unsqueeze(1), (512, 512), interpolation=transforms.InterpolationMode.BILINEAR).float(), reduction="none")
                else:
                    assert False
                ...
                reward_output_loss = reward_output_loss.reshape_as(timestep_mask_readout)
                reward_output_loss = (timestep_mask_readout * reward_output_loss).sum() / (timestep_mask_readout.sum() + 1e-10)
                loss += reward_output_loss * args.readout_beta
                assert args.readout_beta == 1
                

                """
                Rewarding ControlNet each t with reward
                """
                #==========================
                with torch.no_grad():

                    timesteps = (timesteps - 200).clip(1, 1000) #torch.randint(timestep_sampling_start, timestep_sampling_end, (bsz,), device=latents.device)
                    timesteps = timesteps.long()
                    noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)
                    ...
                    # reward model inference
                    if args.task_name == 'canny':
                        blured_labels = reward_model(image.to(accelerator.device), low_threshold, high_threshold)
                    else:
                        blured_labels = reward_model(image.to(accelerator.device))

                    # normalize the predicted depth to (0, 1]
                    if type(blured_labels) == transformers.modeling_outputs.DepthEstimatorOutput:

                        # map predicted depth into [0, 1]
                        blured_labels = blured_labels.predicted_depth
                        blured_labels = torchvision.transforms.functional.resize(blured_labels, (args.resolution, args.resolution), interpolation=transforms.InterpolationMode.BILINEAR)
                        max_values = blured_labels.view(args.train_batch_size, -1).amax(dim=1, keepdim=True).view(args.train_batch_size, 1, 1)
                        blured_labels = blured_labels / max_values

                        mixed_labels = (timesteps.unsqueeze(1).unsqueeze(2) * blured_labels + (1000 - timesteps.unsqueeze(1).unsqueeze(2)) * labels) / 1000
                        # mixed_labels = ((timesteps+100).unsqueeze(1).unsqueeze(2) * blured_labels + (900 - timesteps.unsqueeze(1).unsqueeze(2)) * labels) / 1000
                        # #print(timesteps+100)
                        # assert torch.all(timesteps+100 <= 1000)
                        # assert torch.all(900 - timesteps >= 0)

                    # Avoid nan loss when using FP16 (happen in softmax)
                    # FP32 and BF16 both work well
                    if image.dtype == torch.float16:
                        if isinstance(mixed_labels, torch.Tensor):
                            mixed_labels = mixed_labels.to(torch.float32)
                        elif isinstance(mixed_labels, list):
                            mixed_labels = [x.to(torch.float32) for x in mixed_labels]
                        else:
                            raise NotImplementedError
                ...
                # Determine which samples in the current batch need to calculate reward loss
                    ...
                    # calculate the reward loss
                reward_step_loss = get_reward_loss(outputs, mixed_labels, args.task_name, reduction='none')
                ...
                #print(reward_step_loss.shape, timestep_mask_blured.shape)

                reward_step_loss = reward_step_loss.reshape_as(timestep_mask_blured)
                reward_step_loss = (timestep_mask_blured * reward_step_loss).sum() / (timestep_mask_blured.sum() + 1e-10)
            ...
            logs = {
                ...
                "reward_step_loss": reward_step_loss.detach().item(),
